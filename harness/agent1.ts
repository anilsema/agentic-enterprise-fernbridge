...
export async function runAgent1(memberId: string | null, queryKeyword: string): Promise<Agent1Result> {
  ...
  const estimatedCost = 0.02; // illustrative flat cost for this simple simulated task
  ...
  if (!budgetCheck.allowed) {
    ...
    return { requestId: classified.requestId, tier: "low", status: "refused_budget",
      reason: `Insufficient budget: $${budgetCheck.remainingBudget.toFixed(2)} remaining, needed $${estimatedCost.toFixed(2)}.` };
  }
