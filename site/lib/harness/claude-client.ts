...
/**
 * Next.js context: no manual .env.local loading, see lib/harness/db.ts
 * for why. process.env.ANTHROPIC_API_KEY is populated natively by Next.js
 * in local dev, and by Vercel's Environment Variables in production.
 */
if (!process.env.ANTHROPIC_API_KEY) {
  throw new Error(
    `ANTHROPIC_API_KEY is not set. In local dev, add it to site/.env.local. In production, set it in Vercel's Environment Variables.`
  );
}
